...
export default function InstructorDashboard() {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      const alumnosRes = await fetch("http:///api/usuarios?rol=alumno");
      const alumnosData = await alumnosRes.json();

      const alumnosCompletos = await Promise.all(
        alumnosData.map(async (alumno: Alumno) => {
          const rutinasRes = await fetch(`http:///api/rutinas/${alumno.id}`);
          const progresoRes = await fetch(`http:///api/progreso/${alumno.id}`);
  ...
  const eliminarRutina = async (rutinaId: number) => {
    await fetch(`http:///api/rutinas/${rutinaId}`, {
      method: "DELETE",
    });
